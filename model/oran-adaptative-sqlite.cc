...
NS_LOG_COMPONENT_DEFINE("OranAdaptativeSqlite");

NS_OBJECT_ENSURE_REGISTERED(OranAdaptativeSqlite);

TypeId
OranAdaptativeSqlite::GetTypeId(void)
{
    static TypeId tid = TypeId("ns3::OranAdaptativeSqlite")
                            .SetParent<OranDataRepositorySqlite>()
                            .AddConstructor<OranAdaptativeSqlite>();
    return tid;
}

OranAdaptativeSqlite::OranAdaptativeSqlite()
...
void
OranAdaptativeSqlite::CreateReportTable(Ptr<OranReportSqlite> report)
{
...
std::string
OranAdaptativeSqlite::ParseReportTableInfo(Ptr<OranReportSqlite> report)
{
...
void
OranAdaptativeSqlite::CreateReportSave(Ptr<OranReportSqlite> report)
{
    NS_LOG_FUNCTION(this << report);
    if (m_active)
    {
...
std::string
OranAdaptativeSqlite::CreateReportSaveQuery(Ptr<OranReportSqlite> report)
{
    NS_LOG_FUNCTION(report);
...
OranAdaptativeSqlite::ParseReport(Ptr<OranReportSqlite> report)
{
    NS_LOG_FUNCTION(report);
...
OranAdaptativeSqlite::GetLastReport(const std::string& table)
{
    NS_LOG_FUNCTION(this);
...
std::unordered_map<std::string, std::string>
OranAdaptativeSqlite::GetLastReport(const std::string& table, uint64_t nodeId)
{
    NS_LOG_FUNCTION(this);
    std::unordered_map<std::string, std::string> repData;
    if (m_active)
...
std::unordered_map<std::string, std::string>
OranAdaptativeSqlite::GetCustomQuery(const std::string& query)
{
    NS_LOG_FUNCTION(this);
    std::unordered_map<std::string, std::string> repData;
    if (m_active)
